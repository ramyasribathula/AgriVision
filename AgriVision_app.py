...
# Government of India OGD mandi-price dataset. The resource ID is public;
# only the user's own API key must be supplied as an environment variable.
...
def market_prices_api():
    """Free live mandi-price API. Uses Farmer.in's open JSON endpoint,
    which publishes daily mandi prices sourced from Agmarknet/data.gov.in.
    ...
    market = str(request.args.get("market", "")).strip()

    try:
        response = requests.get(
            "https://farmer.in/api/open/prices.json",
            timeout=20,
            headers={"User-Agent": "AgriVision/1.0", "Accept": "application/json"}
        )
        response.raise_for_status()
        payload = response.json()

        rows = payload.get("prices", payload.get("data", payload if isinstance(payload, list) else []))
        ...
        def matches(value, wanted):
            return not wanted or value.lower() == wanted.lower() or wanted.lower() in value.lower()
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            r = {
                "market": text(row, "market", "Market", "apmc", "APMC", "market_name"),
                "commodity": text(row, "commodity", "Commodity", "crop", "commodity_name"),
                "variety": text(row, "variety", "Variety"),
                "state": text(row, "state", "State", "state_name"),
                "district": text(row, "district", "District", "district_name"),
                "min_price": row.get("min_price", row.get("Min Price", row.get("min", row.get("minPrice")))),
                "modal_price": row.get("modal_price", row.get("Modal Price", row.get("modal", row.get("modalPrice")))),
                "max_price": row.get("max_price", row.get("Max Price", row.get("max", row.get("maxPrice")))),
                "date": text(row, "date", "Date", "arrival_date", "Arrival_Date", "arrivalDate", "price_date")
            }
            if not matches(r["commodity"], commodity):
                continue
            if not matches(r["state"], state):
                continue
            if not matches(r["district"], district):
                continue
            if not matches(r["market"], market):
                continue
            records.append(r)

        return jsonify({
            "success": True,
            "source": "Farmer.in open mandi API (Agmarknet / data.gov.in)",
            "records": records,
            "count": len(records),
            "filters": {"commodity": commodity, "state": state, "district": district, "market": market}
        })
    except requests.RequestException as error:
        logger.warning("Free mandi-price API failed: %s", error)
        return api_error("Live mandi-price service is temporarily unavailable.", 502)
    except ValueError:
        return api_error("The mandi-price provider returned an unreadable response.", 502)
...
if __name__ == "__main__":
    ...
    print("💹 Mandi API: " + ("Configured" if DATA_GOV_API_KEY else "API key required"))
    print("🆔 Mandi resource: " + DATA_GOV_RESOURCE_ID)
    print("=" * 58)
    print()
